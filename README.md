...
| **世界持久化（重启不丢方块）** | ✅ |
| **玩家数据持久化（物品栏/位置，重连保持）** | ✅ 真实 MC 26.2 客户端实测 |
| **多 chunk 视距（动态加载/卸载）+ 平坦世界生成** | ✅ 真实 MC 26.2 客户端实测 |
| Docker 交付（21MB 镜像） | ✅ |
...
- [x] 世界持久化（gob 存盘，重启不丢方块）
- [x] 玩家数据持久化（物品栏 + 位置，重连保持）+ creative 模式
- [x] 多 chunk 视距（动态加载/卸载）+ 平坦世界生成 + 放置碰撞检查
- [x] Docker 镜像交付（21MB）
- [ ] 方块物理（重力/水）（未来）
